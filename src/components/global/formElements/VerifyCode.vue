<template>
	...
	>
		<div class="code-input flex flex-center gap-2 md:gap-3 lg:gap-4 xl:gap-5">
			<template v-for="(v, index) in values" :key="index">
				<input
					class="text-center w-[44px] h-[50px] md:w-[66px] md:h-[72px] border border-solid text-text-500 rounded-md text-xl xl:text-2xl"
					:class="{ 'border-primary-50': v }"
					type="text"
					pattern="[0-9]"
