<template>
  <Box class="p-5">
    <Dialog
      v-model:visible="showClientDetails"
      maximizable
      modal
    ...
    >
      ...
      <div class="mb-4">
        ...
        <span
          >{{ clientDetails.phone }} - {{ clientDetails.secondary_phone }}</span
        >
      </div>
      <div class="mb-4">
    ...
    >
      <main-table
        ...
        :list_data="orderDetails.orderItems"
      ...
      <div
        class="bg-gray-50 rounded-md my-4 p-3"
        v-if="orderDetails.statusHistories.length"
      >
        <h5 class="text-lg">الحالات السابقة</h5>
        <p
          v-for="(i, key) in orderDetails.statusHistories"
          :key="key"
          class="mb-2"
        >
          {{ status_text[i.status] }} :
          {{ i.reason ? i.reason : "لا يوجد تعليق" }}
        </p>
      </div>
      <label class="mb-2 text-gray-700 text-lg">اختر الحالة :</label>
      <div class="gap-4 grid grid-cols-4">
        <div
          :class="['col-span-1 overflow-hidden cursor-pointer']"
          @click="selected = item"
        ...
        >
          <div
            :class="[
              'border w-full text-center py-3 rounded-md',
              { 'bg-primary-200 text-primary-300': selected == item },
            ]"
          >
            {{ status_text[item] }}
          </div>
    ...
    <Dialog
      v-model:visible="selectOrder"
      maximizable
      modal
      header="إضافه منتج للطلب"
      :style="{ width: '50vw' }"
      :breakpoints="{ '1199px': '75vw', '575px': '90vw' }"
    >
      <ValidationForm
    ...
    >
      ...
      <template v-slot:changeStatus="{ data }">
        <app-button
          class="border border-primary-300 !text-primary-300 !bg-white"
          v-if="$hasPer('orders:update')"
          submit-title="تفاصيل الطلب"
...
function change(s) {
  ...
  details.value.final_price = allProducts.value.find(
    (data) => data.id == s
  ).price;
}
function changeStatus() {
  ...
  orderServices
    .changeStatus(orderDetails.value.id, {
